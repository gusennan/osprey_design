...
from osprey_design.globals import ATTR_FOOTER, ATTR_HEADER
...
class FlagFileWidget(urwid.TreeWidget):
    ...
    def flagged(self, val):
        _on_selection_changed(self._path, 'selected' if val else 'unselected')
        self._flagged = val

    def __init__(self, node):
        super().__init__(node)
        self._path = node.get_value()
    ...
    def unhandled_keys(self, size, key):
        ...
        Default behavior: Toggle flagged on space, exits on enter
        """
        if key == " ":
            global _flagged_files
            # Make space for a newly flagged file
            if len(_flagged_files) == _max_selectable and not self.flagged:
                head = _flagged_files[0]
                _flagged_files = _flagged_files[1:]
                head.flagged = False
                head.update_w()
                _flagged_files.append(self)
            elif self.flagged:
                _flagged_files.remove(self)
            else:
                _flagged_files.append(self)
            ...
            if not _flagged_files:
                self.unhandled_keys(size, " ")  # Select the current file
...
class DirectoryBrowser:
    palette = [
        ...
        (ATTR_HEADER, 'yellow', 'black', 'standout'),
        (ATTR_FOOTER, 'light gray', 'black'),
    ...
    footer_text = [
        ...
        "  ",
        ('key', "SPACE (toggles selection of file)"), "  ",
        ('key', "+ (expands directory)"), ",",
        ('key', "- (collapses directory)"), "  ",
        #        ('key', "LEFT"), "  ",
        #        ('key', "HOME"), "  ",
        #        ('key', "END"), "  ",
        ('key', "ENTER (returns to previous page)"),
    ]
    ...
    def __init__(self, max_selectable=-1, file_selection_changed=None):
        """If max_selectable is -1, that means no limit on how many files can be selected.
        file_selection_changed is a callback that passes two arguments, a path and a string indicating
        whether a file was 'selected' or 'unselected'.
        """
        global _max_selectable, _on_selection_changed, _flagged_files
        if max_selectable != -1:
            _max_selectable = max_selectable
        if file_selection_changed is not None:
            _on_selection_changed = file_selection_changed
        _flagged_files = []
        ...
        self.view = urwid.Frame(
            ...
            header=urwid.AttrWrap(self.header, ATTR_HEADER),
            footer=self.footer)
